...
**M0 出口自检清单**：
- [x] `2026-08-16-ffi-spike.md` 对比报告已产出，D14 已冻结（☑ 手写 C ABI / ☐ cxx）
- [x] 工具链脚本 `01_vcpkg.ps1`、`02_build.ps1` 退出码 0（2026-08-17 实测）
- [x] `m0_magnet_e2e` PASS（本地 seeder，60s 内 progress>0；实测 1.4s 下载完成，progress=1.0）
- [x] `torrent_finished → lt_pause` 验证结论写回设计文档 §10.1（作证停止路径，fin<paused 顺序恒定；ABI100 status() 不反映暂停）
- [x] alert ≤12 种字段级 schema 终稿（设计文档 §8.5）
- [x] M1 可开工检查表：btcore 骨架可编译、ASAN 配置就绪（`scripts/m0/03_asan_prep.md`）；lt.h 全量进入 M1 范围
