...
> **状态：2026-08-16 拍板决策（v0.6）→ 2026-08-17 M0–M6 全部交付（v1.0）→ 2026-08-17 M7 偏差收尾（D34 + WS 端点 + 1s 节流）**。
...
> 2. **§12 D36 WS 端点**：v1 交付 WsHub 协议层（9 类事件 + seq + 256 队列背压 + 跳号补拉/重连重同步，测试全覆盖）。**M7（`4d454e3`）补齐 socket 端点**：`GET /ws` 升级路由（axum ws）——连接即推全量（重连重同步）→ 200ms 增量轮询 → **1s 快照节流**（Progress/Speed 按任务合并为最新值，`Throttler` 在 `ws.rs`，跳号由客户端按 D36 用 `GET /tasks/:id` 补拉）；关键事件直通；单消费者语义（D22 本机单用户）。测试：`tests/ws_api.rs` 4 项（含节流时序）。
> 3. **§15/§18 D34 token 剔除**：**M7（`4d454e3`）实现**——`canonical_http_url` 按黑名单（`token|sig|signature|expires|auth|X-Amz-*|X-Goog-*|X-Tencent-*|X-QiNiu-*`）剥离签名参数后作 canonical 身份；同资源不同 token 判重复（409）；非 token 参数差异仍为不同任务。单测 6 + http_api 集成 2。
...
> 5. **§20 决策清单**：✅ 均为"决策已拍板采纳"（决策状态），非功能实现状态；M0–M6 已实现者：O1/O2/O3/O6（自研/CLI+WS/本机/FallbackPolicy）、B1–B15 中核心链路项（状态机/路由/HTTP 引擎/Provider 链路/WS 事件）、D21/D26 CLI 命令集（8 命令+fallback，Q-B9 入口）、D35 最小 CI（`.github/workflows/ci.yml`，btcore 后置）。**M7 已收尾**：1s 快照节流/WS socket 端点（偏差 2）、D34 token 剔除（偏差 3）。**后置项**（已决策未实现，见 TDD 计划后续/运维层）：C 组参数运行时接线、B10 磁盘预检、B14 单实例锁、TOML 冷加载。
