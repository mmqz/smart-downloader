# 迅雷 P2P 网络接入逆向研究 - 完整成果合集

> **生成时间**: 2026-08-17T11:06:15.946558
> **状态**: PHub body 加密格式未完全破解,HTTP 发送链路已完整逆向
> **关键阻碍**: QAClient 协议层对 body 做了额外封装
...
11. PHub HTTP PoC v3
12. PHub HTTP PoC v4
13. XBTPackage vtable 反汇编脚本
14. 加密函数反汇编脚本
15. PHub/SHub 命令分析脚本
16. Http.dll 反汇编脚本
17. Ghidra 反编译脚本
18. alist thunder driver - util.go (captcha_sign + device_sign 算法)
19. alist thunder driver - driver.go (身份常量 + Algorithms)
20. alist thunder driver - types.go
21. XBTPackage vtable 反汇编结果 JSON
22. PHub/SHub 命令分析结果 JSON
...
**路径**: `/home/z/my-project/research/p2p_recon/HTTP_DLL_REPORT.md`

```markdown
# 迅雷 P2P 网络接入 - Http.dll 反汇编报告

## 核心发现

### 1. Http.dll 完整导出函数表 (26 个)

Http.dll 提供完整的 HTTP 请求/响应构造 API:

**HTTP 请求构造:**
- XPF_HttpRequestPackageSetField (设置 header 字段)
- XPF_HttpRequestPackageSetRequestBody (设置 body)
- XPF_HttpRequestPackageSetRequestLine (设置请求行: POST / HTTP/1.1)
- XPF_HttpRequestPackageGetField / GetRequestBody / GetRequestLine / GetUrl / GetVersion / GetRequestType / IsKeepAlive / GetRange

**HTTP 响应解析:**
- XPF_HttpResponseHeaderPackageGetField / GetResponseHeader / GetStatusCode / GetStatusDescription / GetVersion
- XPF_HttpResponseHeaderPackageSetField / SetStatusCode / SetStatusDescription / SetVersion

**初始化/清理:**
- XPF_HttpInit / XPF_HttpUninit

### 2. HTTP 请求构造流程 (A 级)

从字符串 "POST " + " HTTP/1.1" 的引用反汇编:

```
HttpInputChannelSession::InternalGenerateRequestHeaderComplete:
  1. 检查 request type (1 = POST)
  2. "POST " + URL (从 r14+0x1b0 读, std::string) + " HTTP/1.1" 拼接成请求行
  3. 遍历 header 链表 (Package+0x40), 逐个写入 "Field: Value\r\n"
  4. 写入 "Content-Length: N\r\n" (如果有 body)
  5. 写入 "\r\n" (空行结束 header)
  6. 写入 body (从 SetRequestBody 设置的内容)
```

### 3. SetField 实现 (A 级)

XPF_HttpRequestPackageSetField @ 0x180021e30:
  - 调 XPF_PackageGetClassData 获取类数据
  - jmp 0x180023370 (实际实现)

实际实现 @ 0x180023370:
  - 用 XPF_stricmp 做 header 名不区分大小写比较
  - 在链表中查找已有同名字段 (避免重复)
  - 如果找到, 更新值; 如果没找到, 插入新节点

### 4. SetRequestBody 实现 (A 级)

XPF_HttpRequestPackageSetRequestBody @ 0x180022300:
  - 调 XPF_PackageGetClassData
  - 把 body 字符串写入 Package+0x20 (std::string)
  - 计算字符串长度 (strlen)
  
所以 HTTP body 就是 SetRequestBody 设置的字符串/二进制数据!

### 5. HTTP 发送链路 (A 级)

从 DownloadSDK.dll 反汇编:

```
调用者1 (ServicePHubQueryEvent):
  1. AES 加密包 (call 0x180161920)
  2. call HTTP_SEND (0x1802833e0)
     参数: rcx = [rdi+0x38] (HTTP请求对象), rdx = [rdi+0x48] (PhubPkgRequester)

HTTP_SEND (0x1802833e0):
  1. 连接管理 (call 0x180283770)
  2. 状态检查 [rbx+0x9c] (0=未连接)
  3. 连接成功后 call DATA_SEND (0x1802835f0)

DATA_SEND (0x1802835f0):
  1. 循环调用 SEND_LOOP (0x1802836a0)
  2. 检查 [rsi+0x9c] (状态 2=已连接)

SEND_LOOP (0x1802836a0):
  1. 检查 [rsi+0x9c] == 3 (发送状态)
  2. mov rcx, [rsi+0xd0] ← 发送队列
  3. call [rax+8] ← 虚函数,获取包对象
  4. "QAClientPackage" ← 类型标识符
  5. call [rip+0xdf181] ← QueryInterface/类型转换
  6. mov rcx, [rsi+0x78] ← 连接对象
  7. call [rip+0xdf1c1] ← 发送! (XPF_ConnectionPostPackage)
```

### 6. QAClient 协议层 (A 级)

QAClient 是迅雷的**应用层协议封装**:

```
QAClient (客户端)
  ├─ QAClientPackage (包封装)
  │   ├─ XDL_QAClientPackageParser (包解析器)
  │   └─ QAPackageParser (解析器基类)
  ├─ QAPackage (协议包基类)
  └─ QAPackageParserDelegate (解析委托)
```

QAClient 通过 HTTP 连接池发送:
```
HTTPConnectionPool (连接池)
  ├─ AddConnection / Close
  └─ MaxConnectionPerHostPort
```

### 7. 完整发送路径 (A 级)

```
PhubPkgRequester (AES 加密后的二进制包)
  → QAClientPackage (QA 协议封装)
    → HttpRequestPackage (HTTP 请求: 请求行 + headers + body)
      → HTTPConnectionPool (连接池)
        → XPF_ConnectionPostPackage (TCP 发送)
          → pr-phub.sandai.net:80
```

### 8. PHub body 加密格式仍为解决

虽然现在知道了完整的 HTTP 发送链路,但 PHub body 的加密格式仍未破解。

已尝试 15+ 个 PoC, 全部返回 "decrypt request failed":
1. AES-ECB(MD5(cmd+seq), body)
2. RSA(AES key) + AES(body)
3. RSA(整个包)
4. base64 包装
5. form-encoded
6. JSON
...
HTTP 发送链路显示:
1. PHub body 通过 SetRequestBody 设置 → 是字符串
2. 通过 QAClientPackage 封装 → 可能有额外编码
3. 通过 HttpRequestPackage 发送 → 标准 HTTP POST

**最可能原因**: QAClientPackage 在封装时对 body 做了**额外的编码/加密**。

QAClient 的源码路径:
  D:\jenkinsAgent\workspace\Downloadlib_33.2\P2P_Framework_VS2019\source\src\QAClient\UDPConnection\UdpConnection.cpp

QAClient 原本是 UDP 连接! PHub 可能用 QAClient over TCP over HTTP。
QAClient 可能在 HTTP body 外面加了一层 QA 协议头。

### 10. 下一步

1. 反汇编 QAClient 的包构造函数,找 QA 协议头格式
2. 或用 Wireshark 抓真实 PHub 请求
3. 或用 Ghidra 反编译 0x1802836cf (SEND_LOOP) 的完整逻辑
...
| 文件 | 内容 |
|---|---|
| disasm_http_dll.py | Http.dll 完整反汇编脚本 |
| Http.dll 导出函数表 | 26 个导出 (SetField/SetRequestBody/SetRequestLine 等) |
| HTTP 请求构造流程 | InternalGenerateRequestHeaderComplete |
| QAClient 协议层 | QAClientPackage / QAPackageParser |
| 完整发送链路 | PhubPkgRequester → QAClient → HttpRequest → TCP |
```
...
**路径**: `/home/z/my-project/research/p2p_recon/PROGRESS_REPORT_v3.md`
...
**路径**: `/home/z/my-project/research/p2p_recon/RESEARCH_STATE.md`
...
**路径**: `/home/z/my-project/research/p2p_recon/PROGRESS_REPORT_v2.md`
...
**路径**: `/home/z/my-project/research/p2p_recon/FINAL_REPORT.md`
...
### PUBLIC_INTEL_REPORT.md

**路径**: `/home/z/my-project/research/p2p_recon/PUBLIC_INTEL_REPORT.md`
...
### test_captcha_sign.py

**路径**: `/home/z/my-project/scripts/p2p_recon/test_captcha_sign.py`
...
### test_captcha_sign_unit.py

**路径**: `/home/z/my-project/scripts/p2p_recon/test_captcha_sign_unit.py`
...
### poc_phub_http_client.py

**路径**: `/home/z/my-project/scripts/p2p_recon/poc_phub_http_client.py`
...
**路径**: `/home/z/my-project/scripts/p2p_recon/poc_phub_http_v2.py`
...
**路径**: `/home/z/my-project/scripts/p2p_recon/poc_phub_http_v3.py`
...
**路径**: `/home/z/my-project/scripts/p2p_recon/poc_phub_http_v4.py`
...
## XBTPackage vtable 反汇编脚本

### disasm_xbtpackage_vtables.py

**路径**: `/home/z/my-project/scripts/p2p_recon/disasm_xbtpackage_vtables.py`
...
**路径**: `/home/z/my-project/scripts/p2p_recon/disasm_crypto.py`
...
### disasm_phub_shub_cmds.py

**路径**: `/home/z/my-project/scripts/p2p_recon/disasm_phub_shub_cmds.py`
...
**路径**: `/home/z/my-project/scripts/p2p_recon/disasm_http_dll.py`

```python
"""
反汇编 Http.dll 找 QAClientPackage → HTTP body 转换逻辑

目标:
1. 找所有导出函数
2. 找 XPF_HttpRequestPackageSetField 实现
3. 找 HTTP body 序列化函数
4. 找 QAPackage → HTTP 请求的完整转换流程
5. 找加密/编码相关函数
"""
import pefile, capstone, re, json
from pathlib import Path

DLL = "/home/z/my-project/research/extracted/resource_1288_1304_unpacked/Http.dll"
OUT = Path("/home/z/my-project/research/p2p_recon")
OUT.mkdir(exist_ok=True, parents=True)
...
if hasattr(pe, "DIRECTORY_ENTRY_EXPORT"):
    for exp in pe.DIRECTORY_ENTRY_EXPORT.symbols:
        if exp.name:
            n = exp.name.decode("utf-8", errors="ignore")
            exports[n] = exp.address
            print(f"  {n}: {hex(exp.address)} (rva={hex(exp.address)})")
print(f"\n共 {len(exports)} 个导出")
...
keywords = ['HTTP', 'http', 'POST', 'GET', 'Content', 'Body', 'body',
            'QA', 'Package', 'package', 'Encrypt', 'encrypt', 'Decrypt', 'decrypt',
            'Encode', 'encode', 'Decode', 'decode', 'Serialize', 'serialize',
            'Base64', 'base64', 'AES', 'RC4', 'RSA',
...
# 4. 反汇编关键函数
# 重点找:
# - XPF_HttpRequestPackageSetField
# - 任何含 "body" / "serialize" / "encode" 的函数
# - QAPackage 相关函数

# 先看导出函数里有没有和 HTTP body 构造相关的
print("\n=== 反汇编关键导出函数 ===")
target_exports = [n for n in exports if any(kw in n for kw in
    ['SetField', 'SetBody', 'SetData', 'SetContent',
     'Serialize', 'Encode', 'Decode', 'Encrypt', 'Decrypt',
     'Build', 'Construct', 'Send', 'Write',
     'Create', 'Init', 'Open', 'Close',
     'Request', 'Response', 'Package'])]

for name in sorted(target_exports)[:20]:
    rva = exports[name]
    addr = image_base + rva
    code = mem[rva:rva+2000]
    ...
    for ins in insns[:40]:
        ...
        if ins.mnemonic == 'call':
            calls.append(ins.op_str[:60])
    
    if strs or imms or len(calls) > 2:
        print(f"\n  [{name}] @ {hex(addr)} ({len(insns)} insns)")
        if strs:
...
# 5. 深度反汇编 XPF_HttpRequestPackageSetField
print("\n\n=== 深度反汇编 XPF_HttpRequestPackageSetField ===")
if "XPF_HttpRequestPackageSetField" in exports:
    rva = exports["XPF_HttpRequestPackageSetField"]
    addr = image_base + rva
    code = mem[rva:rva+3000]
    insns = list(md.disasm(code, addr))
    print(f"@ {hex(addr)} ({len(insns)} insns)")
    for i, ins in enumerate(insns[:60]):
        ...
        if m:
            ...
            if 0 < val < 0x10000:
                line += f'  ; #{val}'
        m_w = re.match(r'mov\s+(?:dword|word|qword|byte) ptr \[(\w+)\s*\+\s*(0x[0-9a-fA-F]+|\d+)\],\s*(.+)', ins.op_str)
        if m_w:
            line += f'  ; WRITE [{m_w.group(1)}+{m_w.group(2)}]={m_w.group(3)}'
        m_r = re.match(r'(\w+),\s*(?:dword|word|qword|byte) ptr \[(\w+)\s*\+\s*(0x[0-9a-fA-F]+|\d+)\]', ins.op_str)
        if m_r:
            line += f'  ; READ {m_r.group(1)}=[{m_r.group(2)}+{m_r.group(3)}]'
        print(f"  {line}")
        if ins.mnemonic == 'ret' and i > 3:
            break

# 6. 找 QAPackage / QAClient 相关 RTTI
print("\n\n=== QAPackage / QAClient RTTI ===")
rtti_pat = re.compile(rb'\.\?AV[A-Za-z0-9_@]+@@')
for m in rtti_pat.finditer(data):
    s = m.group().decode('utf-8', errors='ignore')
    name = re.sub(r'\.\?AV(.+?)@.*', r'\1', s)
    if 'QA' in name or 'Http' in name or 'Request' in name or 'Package' in name:
        print(f"  {name} @ offset {hex(m.start())}")
```
...
**路径**: `/home/z/my-project/scripts/p2p_recon/ghidra_decompile.py`
...
## alist thunder driver - util.go (captcha_sign + device_sign 算法)

### util_main.go

**路径**: `/home/z/my-project/research/p2p_recon/alist_src/util_main.go`
...
## alist thunder driver - driver.go (身份常量 + Algorithms)

### driver_main.go

**路径**: `/home/z/my-project/research/p2p_recon/alist_src/driver_main.go`
...
## alist thunder driver - types.go

### types_main.go

**路径**: `/home/z/my-project/research/p2p_recon/alist_src/types_main.go`
...
## XBTPackage vtable 反汇编结果 JSON

### xbtpackage_vtables.json

**路径**: `/home/z/my-project/research/p2p_recon/xbtpackage_vtables.json`
      ...
      "methods": [
        {
          ...
          "calls": [
            {
              "insn": "0x1802ebcfa: call 0x18003adb0",
              "target": "0x18003adb0"
            }
          ],
          "mem_writes": [],
...
## PHub/SHub 命令分析结果 JSON

### phub_shub_cmd_analysis.json

**路径**: `/home/z/my-project/research/p2p_recon/phub_shub_cmd_analysis.json`
