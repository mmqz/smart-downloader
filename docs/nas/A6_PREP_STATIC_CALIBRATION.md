...
## 11. 路线裁定：主线回归 Win/Mac 流程，NAS 线归档（用户决策，2026-08-31）

**裁定**：主下载能力走 Win/Mac 流程（provider Xqp0 档），NAS 引擎线降级为
归档文献；A6 实弹从"必做"降级为"可选"。

**依据**：
1. Win/Mac 主线 ≈95% 已建成：登录三模式（设备码/短信/本地页，含官方页
   微信等全方式授权）、直链提取（26MB MD5 逐字节一致）、云盘离线
   （free 3/日独立档）、自有引擎下载（httpdl + libtorrent）——**下载动作
   不经迅雷云端闸门，90120/3 日创建配额天然不适用**。
2. NAS 线唯一增量 = 迅雷私有 P2SP swarm 搬上 Linux 无头设备，属通解矩阵
   L3「私有加速」层（标注"永不通解"）；持续成本（扫码绑定/配额日历/
   平台伪装军备/90120 排障）已超过该增量可兑现的价值。
3. Windows 真机 FFI 全套验证过（Phase 2 交付记录），能力上限高于 NAS 线
   换来的等效物。

**处置清单**：
- A2–A5 校准成果 + 本文 §1–§10 全部保留为研究文献（PR #3 载体）；
- 深水区（launcher report 身份载荷手术）**不做**；
- NAS 引擎工作区 `~/.nas-engine-test` 保留（零成本），SPK 资产入库不动；
- **复活条件**：未来若需要迅雷 swarm 速度于 NAS 场景，从本文档 + PR #3
  证据链 + WORKSPACE.md 冷恢复四命令链整机可复现；复活首件事即 A6
  （1 次扫码 + 1 次额度拿身份/90120 定论）。
