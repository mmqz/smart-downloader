...
async fn main() {
    ...
    let path = std::env::args().nth(1).unwrap_or_else(|| "xunlei_desktop_creds.json".into());
    let mut c: Creds = serde_json::from_str(
        &std::fs::read_to_string(Path::new(&path)).expect("读 creds 失败"),
    )
    .expect("解析 creds 失败");
    ...
    fn jwt_exp(tok: &str) -> u64 {
        ...
        while !s.len().is_multiple_of(4) { s.push('='); }
        serde_json::from_str::<JwtP>(&base64_decode(&s)).map(|j| j.exp).unwrap_or(0)    }
    ...
    if jwt_exp(&c.access_token) < now + 60 && !c.refresh_token.is_empty() {
        ...
        #[derive(Deserialize)] struct TR { #[serde(default)] access_token: String, #[serde(default)] refresh_token: String }
        ...
        let st = r.status(); let body = r.text().await.unwrap_or_default();
        if !st.is_success() { eprintln!("❌ refresh {st}: {}", &body[..body.len().min(300)]); std::process::exit(1); }
        let tr: TR = serde_json::from_str(&body).unwrap();
        c.access_token = tr.access_token;
        if !tr.refresh_token.is_empty() { c.refresh_token = tr.refresh_token; }
    ...
    #[derive(Deserialize)] struct CapR { #[serde(default)] captcha_token: String }
    let cr = http.post(format!("{XLUSER}/v1/shield/captcha/init"))
        ...
        .send().await.unwrap_or_else(|e| { eprintln!("网络错误: {e}"); std::process::exit(1); });
    let cst = cr.status(); let cbody = cr.text().await.unwrap_or_default();
    if !cst.is_success() { eprintln!("❌ captcha/init {cst}: {}", &cbody[..cbody.len().min(300)]); std::process::exit(1); }
    ...
    let resp = http.get(format!("{PAN}/drive/v1/files?parent_id=&usage=DISPLAY&with_audit=true&limit=10"))
