...
mod tests {
    ...
    fn classify_regular_https_is_b() {
        assert_eq!(classify_url("https://example.com/path/file.mkv"), LinkClass::BRegular);
        assert_eq!(classify_url("http://speedtest.tele2.net/1MB.zip"), LinkClass::BRegular);
    }
    ...
    fn classify_cdn_positive_per_region() {
        ...
        for (region, hosts) in cdn_hosts_by_region() {
            ...
            assert_eq!(classify_url(&url), LinkClass::AThunderCdn, "region {} host {} 应判定为 A", region, h);
        }
    }
    ...
    fn classify_n0808_wildcard() {
        assert_eq!(classify_url("https://foo.n0808.com/x"), LinkClass::AThunderCdn);
        assert_eq!(classify_url("https://n0808.com/x"), LinkClass::AThunderCdn);
    }

    #[test]
    fn classify_sandai_wildcard_and_phub() {
        assert_eq!(classify_url("https://hub5p.sandai.net/x"), LinkClass::AThunderCdn);
        assert_eq!(classify_url("https://viphub5pr.phub.sandai.net/x"), LinkClass::AThunderCdn);
    }

    #[test]
    fn classify_bare_host_without_scheme_is_b() {
        // 无 scheme 时无法解析出可信 host，按普通源处理
        assert_eq!(classify_url("vod0001-c01-vip-lixian.xunlei.com"), LinkClass::BRegular);
    }
